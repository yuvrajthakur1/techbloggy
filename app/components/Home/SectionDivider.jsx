...
export default function SectionDivider({ type = "curve", color = "#708993" }) {
  return (
    <div className="relative w-full overflow-hidden -mt-px">
      {type === "wave" && (
        <svg
      ...
      {type === "slant" && (
        <div
          className="w-full h-6 sm:h-10 -skew-y-2"
          style={{ backgroundColor: color }}
        ></div>
      )}
      ...
      {type === "curve" && (
        <div
          className="w-full h-4 sm:h-8 rounded-t-full"
          style={{ backgroundColor: color }}
        ></div>
      )}
    </div>
